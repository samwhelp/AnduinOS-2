...
function run_chroot() {
	print_ok "Running rundown.sh in new_building_os..."
	print_warn "============================================"
...
function umount_folders() {
	...
	print_ok "Cleaning mods from chroot /opt/build ..."
...
function build_iso() {
	...
	REAL_VMLINUZ=$(readlink -f "$DISTRO_IMG_DIR_PATH/vmlinuz" 2>/dev/null)
	[ -f "$REAL_VMLINUZ" ] || REAL_VMLINUZ=$(readlink -f "$DISTRO_IMG_DIR_PATH/boot/vmlinuz" 2>/dev/null)
	REAL_INITRD=$(readlink -f "$DISTRO_IMG_DIR_PATH/initrd.img" 2>/dev/null)
	[ -f "$REAL_INITRD" ] || REAL_INITRD=$(readlink -f "$DISTRO_IMG_DIR_PATH/boot/initrd.img" 2>/dev/null)
...
EOF
	...
	print_ok "Moving iso image to $DIST_DIR_PATH/$TARGET_BUSINESS_NAME-$TARGET_BUILD_VERSION-$DATE.iso..."
	...
	HASH=$(sha256sum "$DIST_DIR_PATH/$TARGET_BUSINESS_NAME-$TARGET_BUILD_VERSION-$DATE.iso" | cut -d ' ' -f 1)
